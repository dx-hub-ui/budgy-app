...
- As notificações de autenticação passaram a chamar `supabase.auth.getUser()` em vez de confiar no usuário retornado por `getSession` ou pelo callback `onAuthStateChange`. Isso elimina sessões forjadas salvas em cookies corrompidos e reduz o erro `Using the user object as returned from supabase.auth.getSession() could be insecure` nos ambientes modernos do Supabase.
