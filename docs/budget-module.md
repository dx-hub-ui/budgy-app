...
- As seeds padrão de categorias chamam `seed_default_budget_categories(p_org_id)` sempre com o `org_id` resolvido. Assim, os grupos e categorias públicos são criados uma única vez por organização e podem ser estendidos pelo usuário sem interferir em outros workspaces. O catálogo `default_categories` garante que instalações futuras recebam o mesmo conjunto base sem depender de listas embutidas em funções.
