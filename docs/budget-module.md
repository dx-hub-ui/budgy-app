...
- A sincronização do slug mensal agora acontece somente durante ações explícitas no planejador (anterior/próximo) quando o usuário está em `/budgets/[slug]`, evitando que o orçamento reescreva a URL enquanto ele navega pela sidebar ou abre `/budgets/report`.
...
A rota `PUT /api/budget/goal/:categoryId` agora encapsula as operações do Supabase em um retry automático (até três tentativas com backoff curto) quando a biblioteca retorna `TypeError: fetch failed`. Isso reduz falhas intermitentes de rede antes que `handleError` devolva 503 para o front. Além disso, a resposta agora valida explicitamente que o Supabase devolveu o registro recém-inserido/atualizado antes de retornar 200; caso contrário, um erro 500 em PT-BR evita que o front receba um corpo `null` silencioso. O mesmo helper `withFetchRetry` é reutilizado nos deletes e na criação de categorias para garantir que selects/inserts reexecutem automaticamente quando a rede oscila, mantendo os tipos do SDK (`PostgrestSingleResponse`/`PostgrestMaybeSingleResponse`) para garantir checagem estática ao compilar o build. A rota `PUT /api/budget/allocation` segue a mesma estratégia: as leituras do mês corrente e anterior, bem como o `upsert`, executam com retry e propagam um erro 503 com a mensagem "Não foi possível conectar ao Supabase" quando a conexão falha, informando ao usuário para revisar as variáveis `NEXT_PUBLIC_SUPABASE_URL` e `SUPABASE_SERVICE_ROLE_KEY`. Até mesmo o bootstrap do esquema (`ensure_budget_category_schema`) agora roda sob `withFetchRetry`, evitando que um `TypeError: fetch failed` abortado pelo SDK interrompa o carregamento inicial do orçamento antes de uma segunda tentativa automática.

## Fluxo de UI/UX

1. **Inicialização** – `useBudgetPlannerStore.initializeMonth` carrega snapshot, popula Zustand e zera histórico (máx. 50 passos).
2. **Cabeçalho do orçamento** – ocupa toda a largura útil dentro do conteúdo principal (sem topbar global) e mantém navegação, resumo e ações em uma única faixa responsiva. As setas de navegação continuam em ícones cinza (#6E6E6E) com hover azul (#5865F2). O cartão "Pronto para atribuir" replica o bloco compacto visto no YNAB: largura útil entre 220 px e 260 px, rótulo em caixa alta com tracking estreito, valor destacado em corpo 24 px e um botão primário "Atribuir" alinhado à direita. A superfície usa verde claro sólido (#DCFCE7) com borda #A7F3D0 e sombra suave (`var(--cc-shadow-2)`), enquanto o botão aplica verde médio (#10B981) com texto branco e foco acessível (`outline` visível). O bloco fica lado a lado com o seletor de mês e os chips de resumo em telas largas, mas quebra a linha com espaçamento consistente em viewports menores. O cabeçalho também conserva o acesso rápido aos grupos de categorias e os botões `Desfazer`/`Refazer` (atalhos `⌘/Ctrl+Z` e `Shift+⌘/Ctrl+Z`).
   - O botão auxiliar que alternava a visibilidade das categorias ocultas foi removido para simplificar o cabeçalho; a gestão dessas categorias agora acontece diretamente no grid por meio do grupo fixo "Categorias Ocultas".
...
4. **Grid de categorias** – accordions por grupo, célula "Atribuído" mostra o valor formatado em BRL e, ao clicar, revela um campo inline para edição. O cabeçalho do grid agora usa tipografia menor (≈11 px) para aproximar o layout da referência do YNAB, enquanto a pill de "Disponível" permanece colorida (`cc-pill-positive`, `cc-pill-zero`, `cc-pill-negative`). Clicar em qualquer linha seleciona a categoria e abre seus detalhes no painel lateral.
   - Toda a linha responde ao clique para carregar o painel lateral. O texto do nome da categoria continua abrindo o modal de edição, mas agora também seleciona a linha, garantindo que o inspector permaneça sincronizado mesmo quando a renomeação é acionada.
   ...
   - Categorias ocultas deixam de aparecer em seus grupos originais e migram para o cartão "Categorias Ocultas" renderizado após o último grupo. O cartão resume até três nomes, indica a contagem total e abre um modal dedicado ao clique.
   - O modal de categorias ocultas replica o layout do mock: lista todas as entradas com ícone/nome/grupo e oferece o botão **Reexibir**, que dispara `PATCH /budget/category/:id` com `is_hidden = false`. O diálogo permanece aberto para reexibir várias categorias em sequência e se fecha automaticamente quando não restarem itens ocultos.
...
6. **Painel lateral da categoria** – passou a espelhar o mock entregue com quatro blocos fixos totalmente localizados em PT-BR:
   ...
   O cabeçalho mostra o ícone da categoria, o botão de edição (pencil) e o atalho "Arquivar" reutilizando o fluxo de ocultar categoria. Um botão "×" limpa a seleção. O painel agora ocupa aproximadamente **20 %** da largura útil (com mínimo de 320 px) e mantém rolagem independente do grid principal, alinhado à nova coluna de categorias que também ganhou scroll próprio.
