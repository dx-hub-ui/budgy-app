...
- **`budget_categories`**: mantém `group_name`, `name`, `icon`, `sort`, flags `is_hidden` e `deleted_at`. A migration `0004_budget_v2.sql` (ou a rotina `ensure_budget_category_schema()` em ambientes legados) garante a criação da estrutura base e semeia automaticamente seis grupos em PT-BR (Contas Fixas, Necessidades, Desejos, Reservas, Dívidas e Receitas). Todas as consultas/leads da API agora apontam para essa tabela pluralizada, que corresponde ao objeto público `public.budget_categories`.
