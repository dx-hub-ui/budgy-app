...
6. **Painel lateral da categoria** – passou a espelhar o mock entregue com quatro blocos fixos:
   1. **Available Balance** – cartão com saldo atual (`available_cents`), rollover do mês anterior e os totais de "Assigned This Month", "Cash Spending" e "Credit Spending" (0 até termos cartões). Os valores de spending são calculados a partir de `account_transactions` para o mês corrente.
  2. **Target** – accordion colapsado por padrão quando não existe meta. Mesmo recolhido, mostra o texto "How much do you need for {CategoryName}?" e o botão **Create Target**; o cabeçalho só recolhe/expande quando o editor não está aberto. O modo criação exibe tabs `Weekly`/`Monthly`/`Yearly`/`Custom`, campo monetário com botões ±1 real, dropdown "By" (inclui "Last Day of Month" e dias específicos) e "Next month I want to…" somente leitura. `Save Target` persiste em `budget_goal` com `cadence`, `type` e `due_day_of_month`; `Delete` só aparece quando já existe registro. Ao salvar, o bloco passa a mostrar headline "Set Aside Another R$X Each Month", a frase "By the {dia} of the Month", o anel de progresso e o CTA amarelo **Assign R$X** que chama `POST /goal/:id/apply`. Abaixo ficam "Amount to Assign This Month", "Assigned So Far" e "To Go", além do link "Edit Target".
   3. **Auto-Assign** – lista expansível com seis ações: Assigned Last Month, Spent Last Month, Average Assigned, Average Spent (médias simples dos últimos 3 meses), Reset Available Amount (ajusta atribuição para zerar o saldo) e Reset Assigned Amount (zera a coluna). Cada linha exibe o valor em BRL usando os dados do snapshot/detalhe e dispara `PATCH /allocation` via store ao clique.
   4. **Notes** – textarea "Enter a note…" persistido em `budget_categories.note` (limite 500 caracteres). O salvamento ocorre no `blur` chamando `PATCH /budget/category/:id` e apresenta feedback "Saving…"/erro inline.
   O cabeçalho mostra o ícone da categoria, o botão de edição (pencil) e o atalho "Archive" reutilizando o fluxo de ocultar categoria. Um botão "×" limpa a seleção. Todos os textos seguem o layout em inglês das capturas (ex.: "Available Balance", "Create Target", "Assign" etc.).
