...
2. **Cabeçalho do orçamento** – ocupa toda a largura útil dentro do conteúdo principal (sem topbar global) e agora usa um cartão branco (#FFFFFF) com tipografia em cinza-escuro (#1E1E1E). As setas de navegação permanecem em ícones cinza (#6E6E6E) e ganham hover azul (#5865F2), enquanto o card central "Pronto para atribuir" foi reduzido para um bloco compacto (largura máx. 320 px) alinhado ao centro com texto centralizado, fundo lima #C6FF7F, tipografia verde-escura (#1C3A0D) e botão "Atribuir" de largura total (verde #3E8E41 com ícone `ChevronDown`). O bloco também mantém o acesso rápido aos grupos de categorias e os botões `Desfazer`/`Refazer` (atalhos `⌘/Ctrl+Z` e `Shift+⌘/Ctrl+Z`).
