...
- O painel lateral reutiliza os estilos `.card`, `.btn-link` e `.ghost-button`. Os botões fantasma seguem estreitos (`height: 34 px`, padding reduzido e raio 9999 px) e agora exibem um ícone encapsulado na lateral esquerda para reforçar a ação disponível de forma consistente em todo o app.
