...
- O cabeçalho agora usa fundo cinza-claro (`#f2f3f5`) para reproduzir a faixa da referência visual e amplia a largura máxima (`--budget-ready-max-width: 260px`) do cartão verde, que passou a organizar conteúdo em linha (texto à esquerda, CTA à direita) com tipografia reduzida (`--budget-label-size: 0.46rem`, `--budget-ready-amount-size: 1.125rem`, `--budget-cta-size: 0.5rem`) para caber confortavelmente no formato horizontal.
