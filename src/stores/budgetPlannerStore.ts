...
function applyHistorySnapshot(set: (updater: (draft: BudgetPlannerState) => void) => void, snapshot: HistorySnapshot) {
  set((draft) => {
...
export const useBudgetPlannerStore = create<BudgetPlannerState>((set, get) => ({
  ...
  async salvarMeta(categoryId, payload) {
  ...
  async editarAtribuido(categoryId: string, valor: number) {
    ...
    const totalsPrev = { ...(state.totalsByMonth[month] ?? { assigned: 0, activity: 0, available: 0 }) };
    ...
    try {
      ...
      set((draft) => {
        ...
        totals.available += response.allocation.available_cents -
          draft.allocations.byCategoryIdMonth[categoryId][month].available_cents;
        totals.assigned = Object.values(draft.allocations.byCategoryIdMonth)
          .map((months) => months[month]?.assigned_cents ?? 0)
          .reduce((acc, value) => acc + value, 0);
...
export const budgetPlannerSelectors = {
  ...
  useTotals: (month: string) =>
    useBudgetPlannerStore((state) => state.totalsByMonth[month] ?? { assigned: 0, activity: 0, available: 0 }),
  useSubfinanciadas: (month: string) =>
    useBudgetPlannerStore((state) => {
      return state.categories.list
        ...
        .filter((item): item is { category: BudgetCategory; falta: number; projection: ReturnType<typeof calcularProjecaoMeta> } => Boolean(item));
    }),
